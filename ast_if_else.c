...
ast_if_else_new()
{
    ...
    obj = g_object_new(XDP_TYPE_AST_IF_ELSE, NULL);
...
{
    assert(self);
    assert(XDP_IS_AST_IF_ELSE(self));
    assert(if_else_block);
    assert(XDP_IS_AST_IF_BLOCK(if_else_block));
...
ast_if_else_set_else_block(AstIfElse *self, AstCodeBlock *else_block)
{
    ...
    assert(XDP_IS_AST_IF_ELSE(self));
    assert(else_block);
    assert(XDP_IS_AST_CODE_BLOCK(else_block));
...
ast_if_else_do_print(AstNode *self, FILE *out)
{
    ...
    AstIfElse *ifelse = XDP_AST_IF_ELSE(self);
    ...
    {
        ast_node_print(XDP_AST_NODE(ifelse->else_block), out);
    }
